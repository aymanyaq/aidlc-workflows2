...
- Copilot reads its CLI home (`COPILOT_HOME`, default `~/.copilot`;
  `AIDLC_COPILOT_HOME` overrides it). A plugin installed from a git or GitHub
  marketplace is copied and recorded in `config.json` `installedPlugins` with
  its `cache_path`. A plugin installed from a local directory marketplace is
  loaded live and recorded only in `settings.json`, as an `enabledPlugins`
  entry whose marketplace is a `directory` source in `extraKnownMarketplaces`;
  AIDLC resolves it through that directory's marketplace manifest in the order
  Copilot reads it (`marketplace.json`, `.plugin/`, `.github/plugin/`,
  `.claude-plugin/`). `enabledPlugins` `false` disables either kind, and a
  disabled live entry the marketplace no longer lists is not an install. The
  harness itself comes from the install's `harness.json`, so Copilot and
  OpenCode, which share `.aidlc`, are told apart without `AIDLC_HARNESS_NAME`.
